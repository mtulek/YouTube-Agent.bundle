	...
	def search(self, results, media, lang):
		...
		if Prefs['yt_pattern'] != '':
			...
			if yt_id and RE_YT_ID.search(yt_id):
				...
				search_string = String.Quote(media.name, usePlus=False)
				try:
					json_obj = JSON.ObjectFromURL(YOUTUBE_VIDEO_SEARCH % (search_string, Prefs['yt_apikey']))

					if json_obj['totalResults'] != 0:
					...
					filename = String.Unquote(media.filename)
					Log('Could not retrieve data from YouTube for: %s' % filename)
